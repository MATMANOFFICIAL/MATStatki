...
void Game::drawMisc(sf::RenderWindow & window, sf::Font & font, int BOARD_OFFSET_X, int BOARD_OFFSET_Y) { //funkcja pisz¹ca poboczne napisy
	...
	{
        sf::Text resetinfo;
        resetinfo.setFont(font);
        resetinfo.setCharacterSize(30);
        resetinfo.setFillColor(sf::Color::Red);
        resetinfo.setStyle(sf::Text::Bold);
		sf::FloatRect textRect = resetinfo.getLocalBounds();
        resetinfo.setOrigin((textRect.left + textRect.width / 2.0f) - 30, textRect.top + textRect.height / 2.0f - 20);
        resetinfo.setPosition(sf::Vector2f(5, BOARD_OFFSET_Y + 380));
        resetinfo.setString("Kliknij r aby zresetowac ulozenie statkow");
...
void Game::handleShipChoiceEvent(const sf::Event& event) { //Funkcja wyboru jakie statki mo¿na postawiæ na planszy
	...
	else if (progress == 1) {  //Statek 5 kratkowy
        if (event.type == sf::Event::KeyPressed) {
